...
**/
    ...
    abstract class Database {
        ...
        private static function dbConnect() {
            ...
            $dbname     = "db_pokedex"; // change this */
            $dbuser     = "gebruiker"; // change this /
            $dbpass     = 'password'; //** change this/
        ...
        public static function insertData($p_sSql, $p_aData = "", $print=false) {
            $pdo = Database::dbConnect();
            $stmt = $pdo->prepare($p_sSql);    // prepare query
        
            if(is_array($p_aData)) {        // add the data
                $stmt->execute($p_aData);    // execute with data
            } else {
                $stmt->execute();            // execute without params
            }
